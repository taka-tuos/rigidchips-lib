...
quat = {
  ...
  __index = {
    ...
    setEularAngles = function(q, x, y, z)
      if vec3.isvec3(x) then x, y, z = x.x, x.y, x.z end
      local c1 = math.cos(y * .5)
      local s1 = math.sin(y * .5)
      local c2 = math.cos(z * .5)
      local s2 = math.sin(z * .5)
      local c3 = math.cos(x * .5)
      local s3 = math.sin(x * .5)
